...
var CONFIG_ROOT = process.env.SIMPLEBOOK_DIR;
if (!CONFIG_ROOT) {
    if (!userHome) {
        console.log(color.red('HOME or SIMPLEBOOK_DIR needs to be defined'));
        process.exit(1);
    }

    CONFIG_ROOT = path.resolve(userHome, '.simplebook');
}
var VERSIONS_ROOT = path.resolve(CONFIG_ROOT, 'versions');


// Init and prepare configuration for simple-bbook-cli
