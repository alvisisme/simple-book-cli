...
function listVersions() {
    ...
    return _.chain(folders)
        .map(function(tag) {
            ...
            // Is it gitbook?
            if (pkg.name != 'simple-book') return null;
...
function loadVersion(version) {
    ...
    .then(function(resolved) {
        ...
        if (!gitbook) throw new Error('simple-book Version '+resolved.tag+' is corrupted');
        return gitbook;
    });
}
...
function linkVersion(name, folder) {
    if (!name) return Q.reject(new Error('Require a name to represent this simple-book version'));
